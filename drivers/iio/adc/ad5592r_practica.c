...
#define ADI_AD5592R_REG_POWER		0xB
				...
				u16 val)
{
	...
	//msg |= (reg << 11) & ADI_AD5592R_ADDR_MASK;
...
static int adi_ad5592r_init(struct iio_dev *indio_dev)
{
	...
	ret = adi_ad5592r_write_ctr(st, ADI_AD5592R_REG_POWER, 
				ADI_AD5592R_MASK_EN_REF);
	if(ret)
	{
		dev_err(&st->spi->dev, "Power reg write fail");
		return ret;
	}
	...
	if(ret)
	{
		dev_err(&st->spi->dev, "ADC Pin reg write fail");
		return ret;
	}
