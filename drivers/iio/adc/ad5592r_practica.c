...
#define ADI_AD5592E_MASK_RB_EN		BIT(6)
#define ADI_AD5592E_MASK__REG_RB	GENMASK(5,2)
...
0x7DAC  // asta ne asteptam sa primim
0XAC7D //asta primim pt ca avem lE
*/
...
{       u16 msg = 0;
      ...
      //  msg |= ((u16)reg << 11) & ADI_AS5592R_ADDR_MASk;
      //  msg |= val & ADI_AS5592R_VAL_MASK;
...
static int adi_ad5592r_write_nop(struct adi_ad5592r_state *st, __be16 *rx)
{
				 ...
				 
{
	...
	msg |= ADI_AD5592E_MASK_RB_EN;
	msg |= FIELD_PREP(ADI_AD5592E_MASK__REG_RB, reg);
	...
	if(ret)
	{
                 dev_err(&st->spi->dev, "Fail to read ctrl at SPI write");
		 return ret;
	}
        ret = adi_ad5592r_write_nop(st, &rx);
	if(ret)
	{
...
MODULE_DESCRIPTION("IIO ADI AD5592R Driver");
