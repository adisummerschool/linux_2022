...
#define ADI_AD5592_MASK_EN_REF		BIT(9) 
#define ADI_AD5592R_REG_RESET		0xF
#define ADI_AD5592R_VAL_RESET		0xAC
...
#define ADI_AD5592R_DEFAULT_ADC_PIN_GFC ADI_AD5592R_MASK_ADC_PIN(0)|\
				  ...
				  u16 *val)
{
	...
	if (ret)
	{
		dev_err(&st->spi->dev, "Fail read ctr reg at SPI write");
		return ret;
	}
	...
	if (ret)
	{
		dev_err(&st->spi->dev, "Fail read ctr reg at nop");
		return ret;
	}
...
static int adi_ad5592r_read_adc(struct iio_dev *indio_dev, u8 chan, u16 *val)
{
	...
	{
		dev_dbg(&st->spi->dev,"ADC chanel exceeds maimum number");
		return -EINVAL;
	}
	...
	if (ret)
	{
		dev_err(&st->spi->dev, "Failed to write sequence register");
		return ret;
	};
	ret = adi_ad5592r_nop(st,NULL);
	if(ret)
	{
		dev_err(&st->spi->dev, "Failed to read ADC in the first map");
		return ret;
	}
	ret = adi_ad5592r_nop(st,&rx);
	if(ret)
	{
		dev_err(&st->spi->dev, "Failed to read ADC in second map");
		return ret;
	}
	...
	dev_info(&st->spi->dev,"ADC response adr = %d",resp);
	dev_info(&st->spi->dev,"ADC response addr = %d",resp_addr);
	if(resp_addr != chan)
	{
		dev_err(&st->spi->dev,"Response dosen't maatch request chan");
		return -EIO;
	}
...
static int adi_ad5592r_update_gain(struct iio_dev *indio_dev, bool double_gain)
{
	...
	if(ret){
		dev_err(&st->spi->dev,"Fai to read range");
		return ret;
	}
...
static int adi_ad5592_preenable (struct iio_dev *indio_dev)
{	
	struct  adi_ad5592r_state *st = iio_priv(indio_dev);
	u16 active_scan;
	u16 msg =0;
	...
	if (ret)
	{
		dev_err(&st->spi->dev, "Fail preemble at SPI write");
		return ret;
	}
	...
	if (ret)
	{
		dev_err(&st->spi->dev, "Fail preemble at nop");
		return ret;
	}
...
static const struct iio_buffer_setup_ops adi_ad5592r_buffer_ops = { 
	.preenable = &adi_ad5592_preenable
};
...
static int adi_ad5592r_reg_acces (struct iio_dev *indio_dev,
...

{
	...
	if(readval){
		...
		dev_err(&st->spi->dev,"read_reg=0x%x",read);
		*readval = read;
		return ret;
	}
...
static const struct iio_info adi_ad5592r_info = {
	...
	.debugfs_reg_access = &adi_ad5592r_reg_acces,
};
...
static int adi_ad5592r_init(struct iio_dev *indio_dev)
{
	...
	ret = adi_ad5592r_write_ctr(st,ADI_AD5592R_REG_ADC_PIN,
	            			ADI_AD5592R_DEFAULT_ADC_PIN_GFC);
	if (ret)
	{
		dev_err(&st->spi->dev,"Adc pin reg write failed Failed");
		return ret;
	}
...
static int adi_ad5592r_probe(struct spi_device *spi)
{
	...
	dev_info(&spi->dev,"AD5592R Driver Problem");
