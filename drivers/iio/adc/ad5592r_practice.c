...
static int adi_ad5592r_preenable(struct iio_dev *indio_dev)
{
	...
	if (ret) {
		dev_err(&(st->spi->dev), "Failed preenabled at SPI write");
		return ret;
	}

	ret = adi_ad5592r_nop(st, NULL);
	if (ret) {
		dev_err(&(st->spi->dev), "Failed preenabled at nop");
		return ret;
	}	
		     ...
		     long mask)
{
	...
	switch (mask) {
		case IIO_CHAN_INFO_RAW:
			ret = adi_ad5592r_read_adc(indio_dev,
						   chan->channel,
						   (u16 *)val);
			if(ret)
				return ret;
...
static int adi_ad5592r_update_gain(struct iio_dev *indio_dev, bool double_gain)
{
	...
	if(ret) {
		dev_err(&(st->spi->dev), "Failed to read range from reg");
		return ret;		
	}
		      ...
		      long mask)
{
	...
	switch(mask) {
		case IIO_CHAN_INFO_HARDWAREGAIN:
			st->double_gain = val;
			return adi_ad5592r_update_gain(indio_dev,
						       st->double_gain);
	}
	return -EINVAL;
};
...
static int ad5592r_probe(struct spi_device *spi)
{
	...
	if (ret) {
		dev_err(&(st->spi->dev), "Failed Init");
		return ret;
	}
